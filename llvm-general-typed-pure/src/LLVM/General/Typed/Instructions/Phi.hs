...
class Phi (f :: * -> *) where
  phi :: ValueOf a => [(f a, SomeLabel)] -> BasicBlock (Value 'Operand a)

instance Phi (Value const) where
  phi :: forall a . ValueOf a => [(Value const a, SomeLabel)] -> BasicBlock (Value 'Operand a)
...
instance Phi AnyValue where
  phi :: forall a . ValueOf a => [(AnyValue a, SomeLabel)] -> BasicBlock (Value 'Operand a)
  phi incomingValues = do
    -- @TODO: make sure we have evaluated all of the values in the list...
