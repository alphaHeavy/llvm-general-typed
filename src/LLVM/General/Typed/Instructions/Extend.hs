...
ext :: forall a b const . (CanExtend (Value const a) (Value const b), BitsOf (Value const a) + 1 <= BitsOf (Value const b)) => ValueOf (Value const b) => Value const a -> BasicBlock (Value const b)
ext = vmap1' f g where
